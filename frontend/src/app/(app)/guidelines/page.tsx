        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {activeGuidelines.map((g, idx) => {
            ...
            return (
              <div
                key={g.id}
                className={`bg-gradient-to-br ${gradientColors[colorIdx]} rounded-2xl p-5 border transition-all duration-200 hover:scale-[1.01] hover:shadow-lg hover:shadow-slate-900/50 animate-slide-up`}
                style={{ animationDelay: `${idx * 80}ms` }}
              >
                <div className="flex items-start gap-3">
                  <div className={`w-10 h-10 rounded-xl bg-slate-800/60 flex items-center justify-center shrink-0 ${iconColors[colorIdx]}`}>
                    {categoryIcons.default}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-white text-base mb-2">{g.title}</h3>
