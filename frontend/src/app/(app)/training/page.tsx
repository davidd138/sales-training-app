...
export default function TrainingPage() {
  ...
  // Auto-save transcript every 30s during call (protects against tab close)
  useEffect(() => {
    if (phase !== 'call' || !conversationId || training.transcript.length === 0) return;
    const autoSaveInterval = setInterval(() => {
      if (training.transcript.length > 0) {
  ...
  // Warn user before closing tab during a call
  useEffect(() => {
  ...
  }, [phase, training.transcript.length]);
